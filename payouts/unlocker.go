...
var (
	big2                 = big.NewInt(2)
	big32                = big.NewInt(32)
	BlockReward *big.Int = new(big.Int).Mul(big.NewInt(9), big.NewInt(1e+18))
...
//add reward changes for akroma https://medium.com/akroma/akroma-coin-supply-5cb692a77e1b

	reward := new(big.Int).Set(BlockReward)
	headerNumber := big.NewInt(candidate.Height)

	// Epoch 1 - Beyond Block 1.2m
	if headerNumber.Cmp(big.NewInt(1200001)) > 0 {
		reward = new(big.Int).Mul(big.NewInt(6), big.NewInt(1e+18))
	}
	// Epoch 2 - Beyond Block ?
	if headerNumber.Cmp(big.NewInt(2200001)) > 0 {
		reward = new(big.Int).Mul(big.NewInt(55), big.NewInt(1e+17))
	}
	// Epoch 3 - Beyond Block ?
	if headerNumber.Cmp(big.NewInt(3200001)) > 0 {
		reward = new(big.Int).Mul(big.NewInt(5), big.NewInt(1e+18))
	}
	// Epoch 4 - Beyond Block ?
	if headerNumber.Cmp(big.NewInt(4200001)) > 0 {
		reward = new(big.Int).Mul(big.NewInt(45), big.NewInt(1e+17))
	}
